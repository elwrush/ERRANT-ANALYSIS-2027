...
Set the API keys in the **zsh environment** (`~/.env` is sourced by `~/.zshrc`, plus direct exports in `~/.zshrc`/`~/.zshenv`) or as environment variables:
...
Note: this project's keys are already set in the zsh env (`~/.env` has `OPENROUTER_API_KEY`, `SUPABASE_URL`, `SUPABASE_ESL_KEY`). If a key is missing, add it to `~/.env` and open a new shell — do not edit this file.

`SUPABASE_URL` and `SUPABASE_ESL_KEY` are optional — without them, student name/class lookup is skipped. Both are set in `~/.env` (zsh env) for this project.
