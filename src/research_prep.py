#!/usr/bin/env python3
"""Fetch sampled records from Supabase, clean HTML, filter, and write per-record JSONs."""
import os
import sys
import re
import json
...
def main():
    ...
    # Fetch all writing records for sampled students
    all_recs = client.table("student_submissions").select("*")\
        .ilike("skill", "Writing")\
    ...
    for rec in all_recs.data:
        ...
        out_path = OUTPUT_DIR / f"{rec['id']}.json"
